 ...
 */
public class Service_TaskTimer extends AsyncTask<String, String, String> {
	...
	public void setTextView1(int textViewId) {
		timer = (TextView) IntroActivity.getInstance().findViewById(textViewId);
	}
	...
	protected String doInBackground(String... params) {
			...
			try {
				...
				if (time >= 10 && time <= 12 && CliCount0_1 >= 3 && lv0_1) {
					...
					updateOftenIntent(rviews,
							CoinBlockWidgetApp.getApplication());
					Log.d("OftenIntent","time % 10 == 9, time : "+time);
		 ...
		 * else {
		 * 
		 * mPref.Ready(); startTime = mPref.ReadLong("startTime", 0);
		 * Log.d("TaskTimer","startTime "+startTime);
		 * 
		 * //make new starttime while task timer is stopped startTime += time;
		 * mPref.WriteLong("startTime", startTime); mPref.CommitWrite();
		 * 
		 * Log.d("TaskTimer","newstartTime "+startTime);
		 * 
		 * 
	...
	private static void updateOftenIntent(RemoteViews rviews, Context context) {
		...
		Intent intent = new Intent(
				String.format(INTENT_OFTEN_FORMAT, mWidgetId));
