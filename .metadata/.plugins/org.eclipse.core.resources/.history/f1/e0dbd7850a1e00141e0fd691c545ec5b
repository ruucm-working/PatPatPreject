...
/**
 * PagerViewTitleStrip is a non-interactive indicator of the current, next,
 ...
 * {@link ViewTitlePagerAdapter#getPageTitleView(int)} in the adapter supplied to
 * the ViewPager.
 */
public class PagerViewTitleStrip extends ViewGroup implements ViewPager.Decor {
...
//    }
    ...
    public PagerViewTitleStrip(Context context) {
        this(context, null);
    }
...
//        mNextText.setText(text);
        ...
        mPrevView.measure(childWidthSpec, childHeightSpec);
        mCurrView.measure(childWidthSpec, childHeightSpec);
    ...
    void updateAdapter(PagerAdapter oldAdapter, PagerAdapter newAdapter) {
        ...
        if (newAdapter != null) {
            //-------------- CHANGE START --------------
            if (!(newAdapter instanceof ViewTitlePagerAdapter)) {
                throw new IllegalStateException("Adapter must extends ViewTitlePagerAdapter.");
            }
            //-------------- CHANGE END ----------------
    ...
    void updateTextPositions(int position, float positionOffset, boolean force) {
        ...
        final int prevWidth = mPrevView.getMeasuredWidth();
        final int currWidth = mCurrView.getMeasuredWidth();
        ...
        final int prevBaseline = mPrevView.getBaseline();
        final int currBaseline = mCurrView.getBaseline();
        ...
        final int alignedPrevHeight = prevTopOffset + mPrevView.getMeasuredHeight();
        final int alignedCurrHeight = currTopOffset + mCurrView.getMeasuredHeight();
        ...
        mCurrView.layout(currLeft, currTop, currRight,
                currTop + mCurrView.getMeasuredHeight());

        final int prevLeft = Math.min(paddingLeft, currLeft - mScaledTextSpacing - prevWidth);
        mPrevView.layout(prevLeft, prevTop, prevLeft + prevWidth,
                prevTop + mPrevView.getMeasuredHeight());
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            int textHeight = mCurrView.getMeasuredHeight();
